...
function setup() {
  ...
  // -place the span tag under the element with id (loop_count)
  loopingText.parent("loop_count");

  startButton = createButton("Start");
  startButton.parent("start_button");
  // -if button is clicked, then start the looping
  ...
  // -if button is clicked, then stop the looping
  ...
  newButton.mousePressed(() => {
    ...
    // -if user enter the correct pattern, then continue
    if (
      ownArray != null &&
